...
Kotlin Arrow поддерживает всю класическую иерархию, переняв её из функциональных языков программирования. Однако, учитывая особенности языка, можно сказать, что в иерархии Kotlin Arrow присутствует лишний элемент иерархии виде `Traversal`. Ввиду отсутствия *Higher-kinded полиморфизма* в языке Kotlin, в нем также отсутствует понятие *аппликативного функтора*.
...
#goal[реализация языкового средства «Оптики» в языке Cangjie, применение которого, сопоставимо по скорости с кодом, написанным без «Оптики».]
