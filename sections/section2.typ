...
Кроме малозначимых итераций @kp3@kp4, был принято предложени по развитию языка, позволяющее работать с KeyPath выражением, как с функциональным типом. @kp5 Особенностью этого предложения было решение о том, что все эффекты порождаемые замыканиями KeyPath будут высчитываться в момент построения замыкания, вместо каждого вызова. Делая таким образом результирующую функию чистой.
```swift
var nextIndex = 0
func makeIndex() -> Int {
  defer { nextIndex += 1 }
