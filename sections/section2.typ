...
Она предоставляет для нас особенный интерес, по причине простоты вывода типов. Так как язык @cangjie является объектно-ориентированным и в нем присутствует отношение подтипизации на типах, ограничивая нас локальным выводом типов. Из этого следует, что в месте применения #gls("optics", display: ["Оптики"]) нам необходимо знать уже все типы `a`, `b`, `s`, `t`. Что в общем случае нельзя сделать, не имея глобального вывода типов и полиморфных переменных. #TODO[пояснить в чем именно проблема (let-полиморфизм). Лучше даже на примерах. Возможно, стоит вынести эти дебаты про выводимость куда-то в другое место]
...
Можно заметить, что различные оптики, перекрывают своей функциональностью друг друга. Так, например, очевидно, что `Lens` это `Affine`, который всегда сфокусирован на одном значении #TODO[Картинка]. Или, например, `Setter` -- самая общая и гибкая "Оптика", позволяющая модифицировать данные. Ниже приведена диаграмма, отображающая иерархию наследования между видами "Оптики".
...
Для реализации, языку требуется наличие Higher-kinded полиморфизма или его имитация. Для работы с оптикой, как с объектом первого класса нужен глобальный вывод типов. #TODO[Пример, с выбором типа функтора в зависимости от его использования потом. Картинка] Иерархия строится с помощью пересечения/объединения ограничений на классы типов функтора.
...
#TODO[Добавить выше тексту]

Учитывая особенности языка, можно сказать, что в иерархии Kotlin Arrow присутствует лишний элемент иерархии виде `Traversal`. Ввиду отсутствия *Higher-kinded полиморфизма* в языке Kotlin, в нем также отсутствует понятие *аппликативного функтора*.
...
#TODO[Пример, где мы достаем iterable сущность с помощью getter, вызываем на ней итератор и получаем по сути тот же Traversal]
==== Scala Monocle
#TODO[Добавить выше тексту]
#TODO[Рассказать про реализации оптики для всего stdlib]
