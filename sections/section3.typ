...
Из иерархии (@classic_hierarchy[Рисунок]) были убраны `Fold`, `Getter` и `Traversal`. `Traversal` (в следствии этого `Fold`) был исключен из иерархии в соответствии с соображениями, которые были обсуждены при анализе Kotlin Arrow (@kotlin_arrow[см. раздел]), они столь же применимы и для @cangjie. `Getter` же, был исключен, так как в @cangjie, в отличие от функционального языка, это понятие уже присутствует (в том числе неизменяемые `property`) и подходящих сценариев применения найдено не было.

В качестве выигрыша мы получаем более простую иерархию, в которой любой существующий вид "Оптики" композируется с любым другим видом "Оптики", тем самым избавляя нас от исключительных случаев
...
В @cangjie первые два аргумента будут входными аргументами функции. В возвращаемом значении возникает неизвестный функтор `f`, который может принимать либо тип `Const<T, X>`, либо тип `Identitiy<T>`. Однако в языке отсутствует higher-kinded полиморфизм и отсутствует перегрузка по возвращаемому значению. Ничего не мешает нам получить функцию ```haskell (a -> f a) -> s -> f s```, однако мы должны заранее знать то, как именно она будет использоваться, для получения значения или для его модификации. Например:
  - разделить функцию на две `createLensReader`, `createLensWriter`
  - дополнительный параметр, определяющий тип функтора `f`.
...
```
Задача прежняя --- заменить "Old" на "New". Был проведен тест, замеряющий результаты вплоть до глубины вложенности 15, однако рассмотрим только глубины 2,3 и 5. (@bench_laarhoven_same[Таблица])
...
#noindent[
Этот способ не является оптимальным, так как "засоряет" область имени объекта и не исключает ошибок с существованием данных функций, когда мы начнем их генерировать. Вместо этого введем понятие *оптического реестра*
...
#block[
*Правило 1* --- `extend SomeT` объявленный в пакете, отличном от пакета объявления типа `SomeT`, не экспортируется
]
#block[
...
- `value` --- первый элемент в цепочке. Если первый он обернут в макрос `@Type/@TypeOf`, то результатом выражения будет объект первого класса, который надо вызвать. В данном контексте, он не обернут, поэтому интерпретируется как аргумент функции `getter` или `setter`.
...
- Метод, возвращающий `OpticsResolver<U>`, соответствующий объекту "Оптики" `x<T, U>`, определенной над типом `T` --- используется, чтобы получить доступ к "Оптике", которой можно продолжить цепочку
...
Благодаря тому, что у нас закрытая иерархия --- мы можем решить проблему работы с неизвестными типами с помощью перегрузок и осмысленных наименований функций:
...
- `composeForward` --- перегрузки для построения `getter` функции. Сигнатуры отличаются только в типе оптических реестров, выступающих, как свидетели типа.
```cangjie composeForward<S, A, B>(_: LensReg<S>, _: LensReg<A>):
        ((S) -> A, (A) -> B) -> (S) -> B```
- `composeBackward` --- перегрузки для построения `setter` функции. Сигнатуры отличаются только в типе оптических реестров, выступающих, как свидетели типа.
