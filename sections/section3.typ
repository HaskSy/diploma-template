...
#noindent[
Так как основная проблема -- в промежуточных структурах, хранящих `getter`/`setter` то будем пытаться максимально от них избавиться. Данный процесс называется *дефорестацией*#TODO[cite walder paper]
...
- Функции для получения объекта для разрешения имен #TODO[Дать этому объекту хорошее имя] и оптических реестров
```cangjie
