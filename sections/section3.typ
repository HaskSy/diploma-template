...
#TODO[Возможно стоит перенести в первую главу и переписать этот кусок]
Для кодогенерации на основе структуры типа данных (#TODO[структурный полиморфизм]), в @cangjie используется механизм deriving, работающий с помощью процедурных макросов. Данный механизм не является расширяемым для разработчиков за пределами стандартной библиотеки, и используется, для генерации интерфейсов.
...
- Все переменные в первичном конструкторе должны представлять собой поля #TODO[Пример ошибки]
...
```
В данном сценарии мы можем возвращать `Prism` выполняющую сопоставление с образцом и в случае успеха, возвращающую кортеж из всех элементов составляющих объект #TODO[Пример]
=== `Lens` для кортежей
Для удобства пользования кортежами, можно предоставить ограниченное количество перегрузок специальной линзы `at(Int64)`, для кортежей различной длины.

Были попытки сделать, гетерогенный список, в который бы транслировались кортежи, для написания одной полиморфной по числу аргументов коржета линзы, однако в @cangjie невозможно выразить числа на уровне типов. #TODO[Показать это как-то и пояснить]
...
#TODO[упомянуть призму для замкнутых иерархий (`as<Subclass>`)? Она не была задизайнена]
...
В данном разделе мы произведем замеры производительности различных моделей и в конечном итоге разработаем собственную модель, стараясь при этом удовлетворить всем требованиям, поставленным перед библиотекой.#TODO[Ссылка на требования в начале главы]
...
=== Дефорестированная оптика и оптические регистры
...
Процесс дефорестации является распространенной компиляторной оптимизацией в функциональных языках программирования. #TODO[Cite deforestation papers] Однако реализация данной оптимизации с нуля в frontend компилятора, является сложной задачей и выходит за рамки данной работы. #TODO[Нужен фактчек, точно ли у нас нет во фронтенде дефорестации]
Предложение -- хранить "Оптику" в уже дефорестированном виде, как одна/две функции. Единственными способами хранения "Оптики" без промежуточных структур являются:
#enum(
...
#noindent[
Этот способ не является оптимальным, так как "засоряет" область имени объекта и не исключает ошибок с существованием данных фукнций, когда мы начнем их генерировать. Вместо этого введем понятие *оптического регистра*


#definition("Оптический регистр")[Специальная, предоставленная библиотекой dummy-структура с полиморфным типом `LensReg<T>` хранящая все "Оптики" соответствующего ей вида определенные для типа `T`]

Перепишем тот пример, что был ранее на пример с регистром:
```cangjie
...
```
Так как "Оптика" была `Lens`, то используем соответствующий регистр `LensReg`. Так как тип переменной `source` равен `A`, то тип расширяемой структуры `LensReg<A>`. Аналогичные регистры существуют для всех видов "Оптики": `IsoReg`, `LensReg`, `PrismReg`, `AffineReg`, `SetterReg`.
...
Таким образом экспорт нашей "Оптики" ограничен видимостью типа `T` указанного в регистре. И мы можем разделять `package-private` и `public` "Оптику" за счет генерации соответствующего интерфейса в месте определения.
...
Использование блоков-расширений позволило нам достичь следующего:
- Дефорестация с целью улучшения инлайнинга
...
#TODO[Можно наверное, расписать второй пункт детальнее. Пример Scala]
...
#TODO[Упомянуть где-то Tardis Monad или сослаться]

Генерация происходит с помощью следующего CPS:
#align(center)[
    #block[#align(left)[
...
- Объект оптического регистра с указанным/выведенным типом
- Вспомогательный объект для разрешения имен "Оптики" (см. в следующем разделе #TODO[ссылка])
...
- Если "Оптика" с предыдущего шага была прочитана из объекта, а не из регистра, то нам это неважно, они уже разложены по переменным.
- и т. д.

В листинге #TODO[Сделать листинг] представлен пример выражения, и сгенерированного из него @cangjie кода.
...
=== Типобезопасность DSL и разрешение имен
...
- Методы, возвращающие оптический регистр, соответствующий виду объекта "Оптики" `x<T, U>`, определенной над типом `T` --- используется, чтобы достать дефорестированную "Оптику" `x`
...
- `composeForward` --- 25 перегрузок для построения `getter` функции. Сигнатуры отличаются только в типе оптических регистров, выступающих, как свидели типа.
```cangjie composeForward<S, A, B>(_: LensReg<S>, _: LensReg<A>):
        ((S) -> A, (A) -> B) -> (S) -> B```
- `composeBackward` --- 25 перегрузок для построения `setter` функции. Сигнатуры отличаются только в типе оптических регистров, выступающих, как свидели типа.
```cangjie composeBackward<S, A, B>(_: LensReg<S>, _: LensReg<A>):
        ((S) -> A, (S, A) -> S, (A, B) -> A, (A) -> B) -> (S, B) -> S```
- Функции для получения объекта для разрешения имен #TODO[Дать этому объекту хорошее имя] и оптических регистров
```cangjie
