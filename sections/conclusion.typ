...
В рамках данной работы был разработан дизайн библиотеки "Оптики" для языка @cangjie на основе существующей теории и реализаций других языков программирования. Для этого были решены следующие задачи:
    ...
    [Рассмотрены различные сценарии, в которых "Оптика" может быть сгенерирована, или предоставлена как модуль библиотеки],
    [Механизм был расширен с поддерживающего генерируемую оптику, до поддержания пользовательской вместе с сохранением типобезопасности и возможности перегрузки по типу структуры],
    [Полученная реализация протестирована на предмет качества инлайнинга]
)

Однако, для достижения работы с "Оптикой", как с объектом первого класса, библиотека все еще использует пары структур, что как показал анализ, сильно бьет по производительнсти. Приведенный дизайн уже может быть, потенциально предоставлен как third-party библиотека для языка, но все еще имеет множество открытых задач, некоторые из них:
