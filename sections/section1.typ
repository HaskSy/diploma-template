...
В данной главе будет дан обзор концепциям и понятиям, существующим на данный момент в языке @cangjie, а также введен основной набор определений, общих для теории языков программирования вцелом.
...
#line(length: 100%)
#definition("Блок-расшинерие")[ механизм, позволяющий добавлять новые сущности к существующим типам без модификации их исходного кода или наследования. ]
```cangjie
extend String {
    func prinsSize() {
        print(this.size)
...
"123".printSize() // 3
```
Блоки расширения могут, как добавлять произвольный код, так и реальзовывать конкретный интерфейс:
```cangjie
interface I1 {
    fucn f1(): Unit
}
