...
#definition("Доменная модель") [ ТODO ]
...
На данный момент, ни семантика языка @cangjie, ни стандартная библиотека, ни сообщество языка не предоставляют удобного интерфейса для сразу нескольких проблем: отсутствует поддержка интерфейса создания новых версий неизменяемых структур данных; нет удобного метода создания копии изменяемого объекта для его последующей модификации, без воздействия на оригинал; оперирование значениями, хранящимися в сложных типах данных, также затруднительно. Наличие пользовательсоко интерфейса для решения данных и подобных проблем, упростило бы написание программ в функциональной парадигме, поддерживаемой языком.
...
Вспомним пример с повышением зарплаты. Декларативное описание данной операции через #gls("optics", display: ["Оптикy"]) выглядело бы так:
```cangjie
