...
Для дальнейшего обсуждения, потребуется ввести набор вспомогательных определений, схожие с теми, что вводятся в предметной области реляционных моделей данных.

#definition("Представление (view)")[ Репрезентация данных, с целью: предоставления интерфейса взаимодействия с ними, изоляции сложности, сокрытия части данных, и т.д. ]
...
Одни и те же данные, в различных контекстах использования, могут возникать под видом различных представлений. Представления тоже являются данными и могут быть транзитивно "обернуты" в другие представления. У пользователя может возникнуть необходимость в поддержании консистентности между представлениям и консистентностю данных, "лежащих" под ними. Обычно, это решается путем обновления данных через представления, что приводит нас к *пробелме View-Update*

#definition("Проблема View-Update")[ Проблема, гласящая, что, в общем случае, нет однозначно определяемой процедуры обновления оригинальных данных через их представление ]
// NOTE: Читать как: "не все отношения, в общем случае, являются инъективными"
// TODO: Пример, где возникает подобная неоднозначность

Несмотря на то, что в оригинале проблема была сформулирована для реляционных баз данных @database_views, она может возникнуть в любой иной задаче, подразумевающей оперирование данными и их представлениями. Пример: сериализация, RPC и т.д.
...
#definition("View-Update фреймворк")[ Абстракция, предоставляющая функциональность генерации процедур обновления данных по представлению и интерфейс для описания процедур обновления в случаях, когда фреймворк не способен их сгенерировать ]

Идея View-Update фреймворка была сформулирована ранее @matsuoka_framework. В частности, были сформулированы идеи, схожие с механизмом оптики для описания реализаций подобного фреймворка @pierce_framework

Добавление View-Update фреймворка на уровне языка/стандартной библиотеки предоставило бы возможности для бесшовного взаимодействия между представлениями различных библиотек. В то же время позволяя разработчикам описывать свои представления независимо друг от друга
