...
В данной главе будет дан обзор концепциям и понятиям, существующим на данный момент в языке @cangjie, а также введен основной набор определений, общих для теории языков программирования в целом.

Целью данной главы не является подробное описание языка @cangjie, но лишь введение терминов и ознакомление с конструкциями и средствами языка, которые понадобятся нам в дальнейшем. Если возникнет необходимость, то имеется возможность ознакомиться со спецификацией языка подробнее @spec_web. Мы же сконцентрируемся лишь на некоторых её частях
...
#definition("Глобальная область видимости")[ область видимости, связанная с сущностями, объявленными на верхнем лексическом уровне файла. ]
#definition("Разрешение имен")[ процесс поиска значения, связанного с идентификатором в текущей области видимости. ]

=== Языковые средства языка Cangjie
Будучи объектно-ориентированным, языком @cangjie имеет такие понятия как классы, интерфейсы, подтипизация, и т.д.

#definition([Структура (`struct`)])[value-тип, имеющий именованные поля, статические функции и методы, не может быть подтипом/надтипом класса, может реализовывать интерфейсы]
#definition("value-тип")[Тип данных, обладающий @cow семантикой]
...
```
#definition("Первичный конструктор (Primary constructor)")[ Основной конструктор класса/структуры. Декларация совпадает и именем класса/структуры. ]
#line(length: 100%)
#definition([`enum`])[неизменяемый value-тип данных, представляющий собой одно из множества связных значений]
```cangjie
enum TimeUnit {
    | Year(Float32)
...
```
#line(length: 100%)
#definition("Кортеж")[неизменяемых value-тип данных, представляющий собой гетерогенную последовательность элементов]
```cangjie
func multiValues(a: Int32, b: Int32): (Int32, String) {
    (a + b, (a - b).toString())
}

main() {
   var (x, y) = multiValues(8, 24)
}
```
#line(length: 100%)
#definition([Класс (`class`)])[Ссылочный тип данных, во многом похож на `struct`, но в отличие от него, может быть отнаследован или быть наследником]
```cangjie
open class C1 {
    func foo(): Unit {
        return
...
```
#line(length: 100%)
Стоит также выделить более продвинутые средства языка, а именно процедурные макросы и блоки-расширения.
#definition("Процедурный макрос")[это функция, которая выполняется на этапе компиляции и преобразует входной поток токенов (исходный код) в новый поток токенов (сгенерированный код). Процедурные макросы пишутся на обычном Cangjie-коде и позволяют выполнять произвольные преобразования над синтаксическим деревом программы.]
```cangjie
macro package macropackage

public macro MacroName(input: Tokens): Tokens {
    quote(println("Hello ${$input.toString()}"))
}

// Применение:
// @MacroName(arbitrary.expression)
```
Для чего используются:
- Генерация кода: Автоматизация рутинных задач (например, интерфейсы ToString, Serializable).
- Уменьшение шаблонного кода: Инъекция повторяющегося кода.
- Создание DSL: Внедрение предметно-ориентированных языков (например, HTML-шаблоны через макросы).

Во многом похожи на процедурные макросы, представленные в языке `Rust`
#line(length: 100%)
#definition("Блок-расширение")[ механизм, позволяющий добавлять новые сущности к существующим типам без модификации их исходного кода или наследования. ]
```cangjie
extend String {
    func prinsSize() {
        print(this.size)
...
"123".printSize() // 3
```
Блоки расширения могут, как добавлять произвольный код, так и реализовывать конкретный интерфейс:
```cangjie
interface I1 {
    fucn f1(): Unit
}
...
#noindent[Наличие программного интерфейса для решения данных и подобных проблем, упростило бы написание программ, полагающихся на персистентность]

В этом разделе мы рассмотрим примеры, на которых заметна нехватка выразительности. Код, который необходимо написать -- достаточно шаблонный, однако интерфейс, за которым этот код мог бы быть скрыт -- отсутствует
...
==== Кортеж
Предположим, что запись из базы данный представляется кортежем. Модификация уже существующего кортежа, подразумевает ручное обращение ко всем аргументам, которые не будут меняться, для их перезаписи в новых кортеж, и лишь единицы из всего набора значений будут изменены
```cangjie
...
```
Проблема усугубляется тем, что нет возможности описать полиморфный, по числу аргументов кортежа метод. То есть, если метод, каким либо образом, зависит на позиции элементов в кортеже, должна быть написана перегрузка для каждого возможного размера кортежа, либо их разумное количество.
...
==== `struct`
Тип данных `struct` можно персистентно обновлять с помощью синтаксиса "через точку", имеющего под собой @cow семантику, однако `struct` не избавлен от проблемы c персистентным обновлением полностью. Покажем примере структуры, у которой все поля помечены `let`:
```cangjie
...
// Рассмотрим немного искусственный, но, тем не менее демонстративный пример. Пускай, мы вызываем функцию `sqlQuery`, выполняющей запрос к реляционной базе данных. В качестве результата мы получаем, массив структур, где каждое поле структуры представляет собой идентификатор столбца, затем, мы хотим обновить во всех полученных записях, все столбцы, имеющие тип `int` и модифицировать запись, заменив `int` на строку, представляющую собой, шестнадцатеричную запись этого числа.
...
Одни и те же данные, в различных контекстах использования, фигурируют в различных представлениях. Конечное представление, в свою очередь, может быть выражено через цепочку из нескольких представлений. В подобной ситуации, у пользователя рано или поздно возникает необходимость в поддержании консистентности между представлениям и консистентностью данных, "лежащих" под ними. Обычно, это решается путем обновления данных через представления, что приводит нас к *проблеме View-Update*
...
#definition(["Оптика"])[
    Абстракция, хранящая информацию о способе доступа, модификации, обхода сложных типов данных
]
@optics, в том виде, в котором она существует в функциональном программировании, предоставляет подобие "изменяемой ссылки" на позицию внутри структуры. Однако, так как объекты в функциональных языках, обычно, неизменяемы, то "модификация" объекта по этой "ссылке" создает новый объект с обновленными значениями.
#figure(
  image("../attachments/figures/persistence_scheme_lens.png", width: 45%),
  caption: [ Cхема обновления значения `Source_1` типа `Source` путем создания копии `Source_2`],
  placement: auto
)

Область применения #gls("optics", display: ["Оптики"]) меняется в объекто-ориентированных языках --- во главе угла находится изменяемое состояние и изменяемые ссылки поддерживаются языком. Однако, если возникает необходимость построить новую версию сущности, без изменения состояния оригинала, то мы либо:
#list(
  [Заменяем сущность на её персистентный аналог],
  [Предоставляем обертку, имеющую персистентный интерфейс]
)

Вспомним пример с сотрудником департамента. Декларативное описание данной операции через #gls("optics", display: ["Оптикy"]) выглядело бы так:
```cangjie
